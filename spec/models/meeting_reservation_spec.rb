...
RSpec.describe MeetingReservation, type: :model do
  ...
  it 'adds an error if there is an overlapping single event' do
    ...
    overlapping_meeting = build(:meeting_reservation, room: room, start_time: start_time - 1.hour, end_time: start_time + 1.hour)
  ...
  it 'adds an error if there is an overlapping recurring event' do
    ...
    overlapping_meeting = build(:meeting_reservation, room: room, start_time: start_time - 1.hour, end_time: start_time + 1.hour, recurring: "{'validations':{'day':[5]},'rule_type':'IceCube::WeeklyRule','interval':1,'week_start':0}")
    overlapping_meeting.valid?
    expect(overlapping_meeting.errors[:base]).to include('A meeting has already been booked at this time.')
  end
end
