  ...
  (function () {
    ...
    window.RecurringSelectDialog = RecurringSelectDialog = (function () {
      function RecurringSelectDialog(recurring_selector) {
        ...
        this.current_rule =
          this.recurring_selector.recurring_select("current_rule");
        this.initDialogBox();
        if (
          this.current_rule.hash == null ||
          this.current_rule.hash.rule_type == null
        ) {
          this.freqChanged();
        } else {
      ...
      RecurringSelectDialog.prototype.positionDialogVert = function (
        initial_positioning
      ) {
        var dialog_height,
          margin_top,
        ...
        if (initial_positioning != null) {
          ...
          return this.inner_holder.trigger(
            "recurring_select:dialog_positioned"
          );
        } else {
          this.inner_holder.addClass("animated");
          return this.inner_holder.animate(
            new_style_hash,
            200,
            (function (_this) {
              return function () {
                ...
                return _this.inner_holder.trigger(
                  "recurring_select:dialog_positioned"
                );
              };
            })(this)
      ...
      RecurringSelectDialog.prototype.save = function () {
        ...
        this.outer_holder.remove();
        return this.recurring_selector.recurring_select(
          "save",
          this.current_rule
        );
      };
      ...
      RecurringSelectDialog.prototype.mainEventInit = function () {
        ...
        this.save_button = this.content
          .find("input.rs_save")
          .on("click tap", this.save);
        return this.content
          .find("input.rs_cancel")
          .on("click tap", this.cancel);
      };
      ...
      RecurringSelectDialog.prototype.freqInit = function () {
        ...
        if (
          this.current_rule.hash != null &&
          (rule_type = this.current_rule.hash.rule_type) != null
        ) {
      ...
      RecurringSelectDialog.prototype.initWeeklyOptions = function () {
        ...
        if (
          ...
          this.current_rule.hash.validations.day != null
        ) {
          $(this.current_rule.hash.validations.day).each(function (index, val) {
            return section
              .find(".day_holder a[data-value='" + val + "']")
              .addClass("selected");
          });
        }
        section
          .off("click", ".day_holder a")
          .on("click", ".day_holder a", this.daysChanged);
        return section.show();
      };
      ...
      RecurringSelectDialog.prototype.initMonthlyOptions = function () {
        ...
        (base1 = this.current_rule.hash.validations).day_of_month ||
          (base1.day_of_month = []);
        (base2 = this.current_rule.hash.validations).day_of_week ||
          (base2.day_of_week = {});
        this.init_calendar_days(section);
        this.init_calendar_weeks(section);
        in_week_mode =
          Object.keys(this.current_rule.hash.validations.day_of_week).length >
          0;
        ...
        section
          .find("input[name=monthly_rule_type]")
          .on("change", this.toggle_month_view);
        return section.show();
      };
      ...
      RecurringSelectDialog.prototype.summaryFetch = function () {
        ...
        this.current_rule.hash["week_start"] =
          $.fn.recurring_select.texts["first_day_of_week"];
        return $.ajax({
          url:
            "/recurring_select/translate/" +
            $.fn.recurring_select.texts["locale_iso_code"],
          type: "POST",
      ...
      RecurringSelectDialog.prototype.init_calendar_days = function (section) {
        ...
        for (num = i = 1; i <= 31; num = ++i) {
          monthly_calendar.append(
            (day_link = $(document.createElement("a")).text(num))
          );
          if (
            $.inArray(num, this.current_rule.hash.validations.day_of_month) !==
            -1
          ) {
        ...
        monthly_calendar.append(
          (end_of_month_link = $(document.createElement("a")).text(
            $.fn.recurring_select.texts["last_day"]
          ))
        );
        end_of_month_link.addClass("end_of_month");
        if (
          $.inArray(-1, this.current_rule.hash.validations.day_of_month) !== -1
        ) {
          end_of_month_link.addClass("selected");
        }
        return monthly_calendar
          .find("a")
          .on("click tap", this.dateOfMonthChanged);
      };
      ...
      RecurringSelectDialog.prototype.init_calendar_weeks = function (section) {
          ...
          if (show_row[index]) {
            ...
            for (
              day_of_week =
                ...
                ref1 =
                  $.fn.recurring_select.texts["first_day_of_week"],
                ref2 = 7 + $.fn.recurring_select.texts["first_day_of_week"];
              ref1 <= ref2 ? j < ref2 : j > ref2;
              day_of_week = ref1 <= ref2 ? ++j : --j
            ) {
        ...
        $.each(
          ...
          function (key, value) {
            return $.each(value, function (index, instance) {
              return section
                .find("a[day='" + key + "'][instance='" + instance + "']")
                .addClass("selected");
            });
          }
        );
        return monthly_calendar
          .find("a")
          .on("click tap", this.weekOfMonthChanged);
      };

      RecurringSelectDialog.prototype.toggle_month_view = function () {
        var week_mode;
        week_mode = this.content
          .find(".monthly_rule_type_week")
          .prop("checked");
      ...
      RecurringSelectDialog.prototype.intervalChanged = function (event) {
        ...
        (base = this.current_rule).hash || (base.hash = {});
        this.current_rule.hash.interval = parseInt(
          $(event.currentTarget).val()
        );
        if (
          this.current_rule.hash.interval < 1 ||
          isNaN(this.current_rule.hash.interval)
        ) {
          this.current_rule.hash.interval = 1;
        }
        return this.summaryUpdate();
      ...
      RecurringSelectDialog.prototype.dateOfMonthChanged = function (event) {
        ...
        (base = this.current_rule).hash || (base.hash = {});
        this.current_rule.hash.validations = {};
        raw_days = this.content
          .find(".monthly_options .rs_calendar_day a.selected")
          .map(function () {
            var res;
            res =
              $(this).text() === $.fn.recurring_select.texts["last_day"]
                ? -1
                : parseInt($(this).text());
            return res;
          });
      ...
      RecurringSelectDialog.prototype.weekOfMonthChanged = function (event) {
        ...
        this.current_rule.hash.validations.day_of_week = {};
        this.content.find(".monthly_options .rs_calendar_week a.selected").each(
          (function (_this) {
            return function (index, elm) {
              var base1, day, instance;
              day = parseInt($(elm).attr("day"));
              instance = parseInt($(elm).attr("instance"));
              (base1 = _this.current_rule.hash.validations.day_of_week)[day] ||
                (base1[day] = []);
              return _this.current_rule.hash.validations.day_of_week[day].push(
                instance
              );
            };
          })(this)
      ...
      RecurringSelectDialog.prototype.template = function () {
        ...
        str =
          "<div class='rs_dialog_holder'> <div class='rs_dialog'> <div class='rs_dialog_content'> <h1>" +
          $.fn.recurring_select.texts["repeat"] +
          " <a class='cursor-pointer' title='" +
          $.fn.recurring_select.texts["cancel"] +
          "' Alt='" +
          $.fn.recurring_select.texts["cancel"] +
          "'></a> </h1> <p class='frequency-select-wrapper'> <label for='rs_frequency'>" +
          $.fn.recurring_select.texts["frequency"] +
          ":</label> <select data-wrapper-class='ui-recurring-select' id='rs_frequency' class='rs_frequency' name='rs_frequency'> <option value='Daily'>" +
          $.fn.recurring_select.texts["daily"] +
          "</option> <option value='Weekly'>" +
          $.fn.recurring_select.texts["weekly"] +
          "</option> <option value='Monthly'>" +
          $.fn.recurring_select.texts["monthly"] +
          "</option> <option value='Yearly'>" +
          $.fn.recurring_select.texts["yearly"] +
          "</option> </select> </p> <div class='daily_options freq_option_section'> <p> " +
          ...
          " </p> </div> <div class='weekly_options freq_option_section'> <p> " +
          $.fn.recurring_select.texts["every"] +
          ...
          ": </p> <div class='day_holder'>";
        for (
          day_of_week =
            i =
            ref =
              $.fn.recurring_select.texts["first_day_of_week"],
            ref1 = 7 + $.fn.recurring_select.texts["first_day_of_week"];
          ref <= ref1 ? i < ref1 : i > ref1;
          day_of_week = ref <= ref1 ? ++i : --i
        ) {
          day_of_week = day_of_week % 7;
          str +=
            "<a href='#' data-value='" +
            day_of_week +
            "'>" +
            $.fn.recurring_select.texts["days_first_letter"][day_of_week] +
            "</a>";
        }
        return (str +=
          "</div> <span style='clear:both; visibility:hidden; height:1px;'>.</span> </div> <div class='monthly_options freq_option_section'> <p> " +
          $.fn.recurring_select.texts["every"] +
          ...
          ": </p> <p class='monthly_rule_type'> <span><label for='monthly_rule_type_day'>" +
          $.fn.recurring_select.texts["day_of_month"] +
          "</label><input type='radio' class='monthly_rule_type_day' name='monthly_rule_type' id='monthly_rule_type_day' value='true' /></span> <span><label for='monthly_rule_type_week'>" +
          $.fn.recurring_select.texts["day_of_week"] +
          "</label><input type='radio' class='monthly_rule_type_week' name='monthly_rule_type' id='monthly_rule_type_week' value='true' /></span> </p> <p class='rs_calendar_day'></p> <p class='rs_calendar_week'></p> </div> <div class='yearly_options freq_option_section'> <p> " +
          $.fn.recurring_select.texts["every"] +
          ...
          " </p> </div> <p class='rs_summary'> <span></span> </p> <div class='controls'> <input type='button' data-wrapper-class='ui-recurring-select' class='rs_save' value='" +
          $.fn.recurring_select.texts["ok"] +
          "' /> <input type='button' data-wrapper-class='ui-recurring-select' class='rs_cancel' value='" +
          $.fn.recurring_select.texts["cancel"] +
          "' /> </div> </div> </div> </div>");
      };
  ...
  (function () {
    ...
    let methods = {
      set_initial_values: function () {
        ...
        return this.data(
          "initial-value-str",
          $(this.find("option").get()[this.prop("selectedIndex")]).text()
        );
      },
      changed: function () {
        if (this.val() === "custom") {
    ...
    $.fn.recurring_select = function (method) {
      if (method in methods) {
        ...
        return $.error(
          "Method " + method + " does not exist on jQuery.recurring_select"
        );
      }
    };
